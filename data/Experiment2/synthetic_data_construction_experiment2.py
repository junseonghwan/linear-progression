...
if __name__ == '__main__':
    ...
    if 'Experiment2' not in os.listdir():
        os.mkdir('./Experiment2/')

    ## Experiment2 without passengers:
    if 'Without_passengers' not in os.listdir('./Experiment2/'):
        os.mkdir('./Experiment2/Without_passengers/')
    for k in np.arange(3, 10):
        if str(k) not in os.listdir('./Experiment2/Without_passengers/'):
            os.mkdir('./Experiment2/Without_passengers/{}/'.format(k))
        synthetic_data_construction_fer(
            ...
            model_length=k,
            pw_capacity=None,
            error_param_list=[0.05],
            n_patients=500,
            output_path='./Experiment2/Without_passengers/{}/'.format(k)
        )

    ## Experiment2 with passengers:
    if 'With_passengers' not in os.listdir('./Experiment2/'):
        os.mkdir('./Experiment2/With_passengers/')
    for k in np.arange(3, 10):
        if str(k) not in os.listdir('./Experiment2/With_passengers/'):
            os.mkdir('./Experiment2/With_passengers/{}/'.format(k))
        synthetic_data_construction_fer(
            number_of_datasets=100,
            PE=True,
            n_genes=100,
            ...
            error_param_list=[0.05],
            n_patients=500,
            output_path='./Experiment2/With_passengers/{}/'.format(k)
    )
